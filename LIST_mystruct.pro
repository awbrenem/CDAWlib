...
;
FUNCTION wrt_hybd_strct, a, unit, convar, maxrecs, depend0, mega_num, csv
            ...
            for jindex2=0, (datsize-1) DO BEGIN
              ...
              for ii=0, (n_tags(a.(jindex2))-1) DO BEGIN
                ...
                if varattsreached EQ 'true' then begin
                  jvar = create_struct(jtags[ii], a.(jindex2).(ii))
                  if (counter EQ 0) then varstruct = create_struct(jvar) else varstruct = create_struct(varstruct, jvar)
                  counter = counter + 1
                endif
              endfor
...
for mega_loop=1, mega.num do begin
   ...
   FOR varcheck = 0, N_ELEMENTS (namest) - 1 DO BEGIN
       ...
       IF  STRUPCASE (a.(varcheck).depend_0) ne STRUPCASE (depend0) THEN BEGIN
           ...
           PRINT, 'STATUS= Variable ' + a.(varcheck).varname + ' is not NRV and does not have a ' + $
...
;      endif
   ...
   if csv1 GE 2 then nogatt = 1 ;CWG - Added so that listing headers won't be created when json files are made  
   if(nogatt eq 0) then  status=wrt_hybd_strct(glbatt,unit,0,maxrecs,depend0,mega.num,csv)
   if(norv   eq 0) then  status=wrt_hybd_strct(rvars,unit,1,maxrecs,depend0,mega.num,csv)  
   if(nonrv  eq 0) then  status=wrt_hybd_strct(nrvars,unit,2,maxrecs,depend0,mega.num,csv)
   if(no2drv eq 0) then  status=wrt_hybd_strct(rvars,unit,3,maxrecs,depend0,mega.num,csv)
   if(no3drv eq 0) then  status=wrt_hybd_strct(rvars,unit,4,maxrecs,depend0,mega.num,csv)
   if(noimg  eq 0) then  status=wrt_hybd_strct(rvars,unit,5,maxrecs,depend0,mega.num,csv)
   ; RCJ 07/19/2013  After no4drv data has been reformed, the call below is just like the 
   ;     call above, for 'noimg'.
   if(no4drv eq 0) then  status=wrt_hybd_strct(rvars,unit,5,maxrecs,depend0,mega.num,csv)
   ; Free Memory  
   spdf_delete, rvars
...
if (csv1 EQ 2) then begin  
  ...
  openw, unit1, jsonfilename, /get_lun
