...
;    help,t_d,cdfepochtype
    ...
    if (ibreak) then begin ;break matrix in components
		...
		cmb_timebin_array,t_d, value, tbeg, tend, dtbin, t_out, valueout, fillflag, valueoutuncertaintyofmean $
		...
		cmb_timebin_array,t_d, values, tbeg, tend, dtbin, t_out, valueout ,fillflag, valueoutuncertaintyofmean $
		,serout_flag=valueout_nbin, fill_empty_bins=fill_empty_bins, autobad=autobad, sigmul=sigmul $
		,VALIDMIN=VALIDMIN, VALIDMAX=VALIDMAX
    endelse
...
for ii = 0, nvars-1 do begin ;add by SAB 4/17/2014
    ...
    ; Do not run the function cmb_cdf_check_if_dependencies_are_time_varying() on variables
    ; where ALLOW_BIN is set to 'FALSE'.
    ; Added by Ron Yurow.  
    IF cmb_tag_name_exists('ALLOW_BIN',d.(ip)) THEN BEGIN
       IF d.(ip).allow_bin eq 'FALSE' THEN CONTINUE
    ENDIF
