;Notes - master cdfs are cdfs NOT netcdfs - CDFS.  
;
;$Author: johnson $
;$Date: 2018/11/13 16:55:23 $
;$Header: /home/cdaweb/dev/control/RCS/read_mynetcdf.pro,v 1.7 2018/11/13 16:55:23 johnson Exp kovalick $
;$Locker: kovalick $
;$Revision: 1.7 $
;
...
print, 'reading ',vname, ' for offset ',start_rec,' recs ',recs
print, 'dimension ids ', vinfo.dim, '# dims ',vinfo.ndims
...
;our use.
  ...
  if (ncdf_attexists(NCDFid, 'FIELDNAM', varname=vinfo.name)) then begin ;TJK add call to new routine
      ...
      ;for GOLD, we're given a time for each pixel, for CDAWeb we just need one value so use the 1st good
      ;time for each record.  Have to exclude/ignore the '***' fill values.
      newdat = strarr(recs)

      if (fieldname eq 'Universal Time' and mission_group eq 'GOLD') then begin
;original idea, delete these 3 lines, when we have something better
;      bad = where(strpos(dat, '2') ne 0, nbad) ;find all "fill" values - "2",, e.g. 2018, should be the 1st number in a good date value
;      dat[bad] = '9999-12-31T23:59:59.999999999' ; replace the star values with the TT2000 fill value.
;      dat2 = dat[*]

         for t = 0, recs-1 do begin
            temp = dat[*,*,t] ; each record
            good = where(strpos(temp, '2') eq 0, ngood) ;find all good values: "2",, e.g. 2018, should be the 1st number in a good date value
            if ngood gt 0 then newdat[t] = temp[good[0]] $; 1st good value for this record
                          else newdat[t] = '9999-12-31T23:59:59.999999999' ;fill for tt2000
         endfor
;help, newdat
;stop;         
         temp = 0 ; clear out
         tt2000_times = CDF_PARSE_TT2000(newdat)
         dat = tt2000_times
         tt2000_times = 0 ; clear out
...
;   vinfo = NCDF_VARINQ(NCDFindex,j,/ZVARIABLE)
   ...
   if keyword_set(VAR_TYPE) then begin ; only get VAR_TYPE='data'
      ...
      if (ncdf_attexists(NCDFindex, 'VAR_TYPE', varname=vinfo.name)) then begin ;TJK add call to new routine
        ncdf_attget,NCDFindex,vinfo.name,'VAR_TYPE',attgot
        if ((attgot eq VAR_TYPE) and (vary eq 'VARY')) then vnames=[vnames,vinfo.name]
     endif
   endif else begin
...
;is a master.  For netcdf data files, we will be merging a cdf master, if it exists
...
if ((n_elements(cnames) ge 2) and strpos(cnames[0],'00000000') ne -1) then begin  
      ...
      if keyword_set(ALL) then begin
         if all eq 1 then data_vnames = get_allvarnames_netcdf(NCDFid=data_NCDFid)
         if all eq 2 then data_vnames = get_allvarnames_netcdf(NCDFid=data_NCDFid,var_type='data')
	   ...
	   ;if attributes are global_scope 
		...
		if (base_struct.(vars).(bnum)[0] ne '') then begin ;global attribute isn't blank in master...
              	;if 1st attribute 
              		if (bnum eq 0) then new_struct = create_struct(bnames[bnum],base_struct.(vars).(bnum)) else $
                	new_struct = create_struct(temporary(new_struct), bnames[bnum],base_struct.(vars).(bnum))		
	        endif else begin ;attribute IS blank in master get value for data ncdf
...
;		      print, 'DEBUG Setting missing attribute value ', bnames[bnum], ' = ',atmp.(s[0]), ' from value found in 1st data ncdf'
                        ...
                        endelse
		    endif ;value is good in data ncdf
...
print, 'closing netcdf in merge_metadata_netcdf'
...
print, 'DEBUG asking for number of records ',rec_count
...
;definitely not correct. 
              ...
              data = read_myvariable_netcdf(vnames[vx],NCDFid,$
                                            vary,dtype,recs,start_rec=start_rec, $
                                            rec_count=rec_count, debug=debug,set_column_major=to_column) ; read the data
              ncdftyp[vx] = dtype ;11/27/2018 TJK added this here because for GOLD netcdfs were converting charcter string times to tt2000
