;$Author: ryurow $
;$Date: 2019/03/22 21:24:17 $
;$Header: /home/cdaweb/dev/control/RCS/read_myCDF.pro,v 1.367 2019/03/22 21:24:17 ryurow Exp ryurow $
;$Locker: ryurow $
;$Revision: 1.367 $
;
...
FUNCTION RECAST_CDF_TYPE, val, new_type
    ...
    IF  convert_to_date THEN BEGIN
        ...
        ; are accepted. Successfully parseing a date string results in a
...
CDFid = cdf_open(cdfname, /readonly)
...
; Create am array to hold a flag for each variable indicationg if is a 
; cloned variable.
...
;depend_0 aren't saved into the dhids structure below)
            ...
            ; data seperately as its data is read from a different variable.   Note that we don't 
...
;      findex = tagindex('FUNCTION', vartags) ; find the FUNCTION index number
      ...
      if (findex[0] ne -1) then begin ;found a virtual value w/ a function definition
         ...
         'reorder':   begin
                         burley = reorder (temporary(burley), orig_names, index=vindex)
                      end
         'composite_tbl': begin
